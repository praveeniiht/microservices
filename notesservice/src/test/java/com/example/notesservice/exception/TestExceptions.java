...
public class TestExceptions {
	  ...
	  @Test public void testAddNote() throws Exception { NotesDto notesdto = new
	  NotesDto(); notesdto.setAuthor("Praveen");
	  when(noteService.addNote(notesdto)).thenReturn(notesdto); RequestBuilder
	  requestBuilder = MockMvcRequestBuilders.post("/notesservice/add")
	  .content(MasterData.toJson(notesdto))
	  .contentType(MediaType.APPLICATION_JSON) .accept(MediaType.APPLICATION_JSON);
	  MvcResult result = mockMvc.perform(requestBuilder).andReturn(); int x =
	  result.getResponse().getStatus(); yakshaAssert(currentTest(),x==404? true :
	  false,exceptionTestFile); }
	  
	  @Test public void testDeleteNote() throws Exception { NotesDto notesdto = new
	  NotesDto(); notesdto.setId(1001);
	  when(noteService.addNote(notesdto)).thenReturn(notesdto); RequestBuilder
	  requestBuilder = MockMvcRequestBuilders.get("/notesservice/delete")
	  .content(MasterData.toJson(notesdto))
	  .contentType(MediaType.APPLICATION_JSON) .accept(MediaType.APPLICATION_JSON);
	  MvcResult result = mockMvc.perform(requestBuilder).andReturn(); int x =
	  result.getResponse().getStatus(); yakshaAssert(currentTest(),x==404? true :
